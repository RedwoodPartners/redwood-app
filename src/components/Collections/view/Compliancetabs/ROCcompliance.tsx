...
const RocCompliance: React.FC<RocComplianceProps> = ({ startupId }) => {
  ...
  useEffect(() => {
    const fetchComplianceData = async () => {
      try {
        const response = await databases.listDocuments(DATABASE_ID, ROC_ID, [
  ...
  const handleSaveCompliance = async () => {
    ...
    try {
      ...
      await databases.updateDocument(DATABASE_ID, ROC_ID, editingCompliance.$id, updateData);
  ...
  const handleDeleteCompliance = async () => {
    if (!editingCompliance) return;
    try {
      await databases.deleteDocument(DATABASE_ID, ROC_ID, editingCompliance.$id);
